...
class Model(object):
    def __init__(self, learning_rate):
        ...
        "w1": nn.parameter(np.random.uniform(-0.1, 0.1,[512, 256])),
        "b1": nn.parameter.zeros([256]),
        
        "w2": nn.parameter(np.random.uniform(-0.1, 0.1,[256, 128])),
        "b2": nn.parameter.zeros([128]),         
        
        "w3": nn.parameter(np.random.uniform(-0.1, 0.1,[128, n_classes])),
    ...
    def forward_propagation(self, x):
        ...
        y3 = nn.matmul(y2, self.param["w2"]) + self.param["b2"]
        y3 = nn.relu(y3)

        logits = nn.matmul(y3, self.param["w3"]) + self.param["b3"]
        #######################################################################
    ...
    def get_loss(self, x, y):
        ...
        loss, probs = nn.softmax_cross_entropy_loss(logits, y)
        #######################################################################
    ...
    try:
        ...
        for epoch in range(1, MAX_EPOCHS+1):
            ...
            for step, (x, y) in enumerate(DB.train, 1):            
                ...
                global_step += 1
                n_correct = model.get_correct_predictions(prob, y).item()
                #print(n_correct)
                accuracy = n_correct
