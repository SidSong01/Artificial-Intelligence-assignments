...
# attribution to Clemson University and the authors.# search.py
# ---------
# Licensing Information:  You are free to use or extend these projects for
# educational purposes provided that (1) you do not distribute or publish
# solutions, (2) you retain this notice, and (3) you provide clear
        ...
        for child in children:
            ...
            if child not in obstacles and child_row >= 0 and child_col >= 0 and child_row < n_rows and child_col < n_cols:       
                if child not in closed_set and child not in open_set:
                    action = (child_row - currentNode_row, child_col - currentNode_col)
                    if action == (-1, 0):
                    ...
                    if child == goal:# child reaching the goal?
                        ...
                        movement.reverse()
                        closed_set.add(child)# for the same outcome with the demo video provided
...
#############################################################################
    ...
    while open_set:
        ...
        if currentNode == goal:# reach the goal, start backtrace
            closed_set.add(currentNode)# for the same outcome with the demo video provided
            p_row, p_col = goal
            while (p_row, p_col) != start:
                movement.append(actions[p_row][p_col])
                p_row, p_col = parent[p_row][p_col]
        ...
        currentNode_row, currentNode_col = currentNode
        # get the children
        children = [((currentNode_row-1, currentNode_col),v+costFn((currentNode_row-1, currentNode_col))),
                    ((currentNode_row, currentNode_col-1), v+costFn((currentNode_row, currentNode_col-1))), 
                    ((currentNode_row+1, currentNode_col),v+costFn((currentNode_row+1, currentNode_col))),
                    ((currentNode_row, currentNode_col+1),v+costFn((currentNode_row, currentNode_col+1)))]
        ...
        for child in children:
            ...
            if (child_row, child_col) not in obstacles and child_row >= 0 and child_col >= 0 and child_row < n_rows and child_col < n_cols:
                ...
                elif child in open_set and open_set.get((child_row,child_col)) > cost:
...
#############################################################################
    ...
    while open_set:
        ...
        # get the children
        children = [((currentNode_row-1, currentNode_col),g_cost+costFn((currentNode_row-1, currentNode_col))+heuristic(currentNode_row-1, currentNode_col)),    
             ((currentNode_row, currentNode_col-1), g_cost+costFn((currentNode_row, currentNode_col-1))+heuristic(currentNode_row, currentNode_col-1)), 
             ((currentNode_row+1, currentNode_col),g_cost+costFn((currentNode_row+1, currentNode_col))+heuristic(currentNode_row+1, currentNode_col)),
             ((currentNode_row, currentNode_col+1),g_cost+costFn((currentNode_row, currentNode_col+1))+heuristic(currentNode_row, currentNode_col+1))]
