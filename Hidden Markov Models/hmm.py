...
def distance(p1, p2):
    '''
    calculate the euclidiea distance betweent points
    '''
...
class ExactInference(Inference):
    ...
    def observe(self,
        ...
        for num_r in range(self.n_rows):
            for num_c in range(self.n_cols):
                belief_temp = 1
                for i, mark in enumerate(landmarks):
                    point = self.get_coordinate(num_r, num_c)
                    dist = distance(point,mark)
                    belief_temp *= normal_pdf(observed_distances[i], 
                                     App.SENSOR_NOISE, dist)
                    #'''
                self.belief[num_r][num_c] *= belief_temp
        #belief_temp = self.normalize(belief_temp)
        self.belief = self.normalize(self.belief)
    ...
    def timeUpdate(self):
        ...
        """
        # Please finish the code below 
        belief_temp = [[0]*self.n_cols for _ in range(self.n_rows)]
        for num_r in range(self.n_rows):
             for num_c in range(self.n_cols):
                 new = self.transition_model(num_r, num_c)
                 #print(new, 'end')
                 for n in new:
                     p, (new_r,new_c) = n
                     belief_temp[new_r][new_c] += self.belief[num_r][num_c] * p
        self.belief = self.normalize(belief_temp) 
...
class ParticleFilter(Inference):
    ...
    def observe(self,
        ...
        """
        weight_list = [None]*self.NUM_PARTICLES
        for i_m, m in enumerate(landmarks):
            for i, (r,c) in enumerate(self.particles):
                point = self.get_coordinate(r,c)
                dist = distance(point, m)
                if weight_list[i]:
                    weight_list[i] *= normal_pdf(observed_distances[i_m],
                                                App.SENSOR_NOISE,dist)
                else:
                    weight_list[i] = normal_pdf(observed_distances[i_m],
                                                App.SENSOR_NOISE,dist)
        #print(weight_list, 'end\n')
        self.particles = random.choices(self.particles, weights = weight_list, k = self.NUM_PARTICLES)
        self.updateBelief()  
    ...
    def timeUpdate(self):
        ...
        for i, p in enumerate(self.particles):
            new = self.transition_model(p[0], p[1])
            random_num = random.uniform(0, 1)
            pre = 0
            for n in new:
                p, position = n
                if random_num > pre and random_num < p + pre:
