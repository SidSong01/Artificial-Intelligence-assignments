...
###############################################################################
    def value(player, board, depth_limit):
        ...
        if player == max_player:
            score, placement = max_value(max_player, board, depth_limit)
        if player == min_player:
            score, placement = min_value(min_player, board, depth_limit)
    ...
    def max_value(player, board, depth_limit):
        ...
        for child in get_child_boards(player, board):
            child_score, _ = value(min_player, child[1], depth_limit - 1)
    ...
    def min_value(player, board, depth_limit):
        score = math.inf
        for child in get_child_boards(player, board):
            child_score, _ = value(max_player, child[1], depth_limit - 1)
            if child_score < score:
                score = child_score
                placement = child[0]
...
###############################################################################
    def value(player, board, depth_limit, alpha, beta):
        if board.terminal() or depth_limit == 0:
            ...
            return evaluate(max_player, board), None
        if player == max_player:
            score, placement = max_value(max_player, board, depth_limit, alpha, beta)
        if player == min_player:
            score, placement = min_value(min_player, board, depth_limit, alpha, beta)
    ...
    def max_value(player, board, depth_limit, alpha, beta):
        score = -math.inf
        for child in get_child_boards(player,board):
            child_score, _ = value(min_player, child[1], depth_limit - 1, alpha, beta)
            if child_score > score:
                score = child_score
                placement = child[0]
            if score >= beta:
    ...
    def min_value(player, board, depth_limit, alpha, beta):
        score = math.inf
        for child in get_child_boards(player, board):
            child_score, _ = value(max_player, child[1], depth_limit - 1, alpha, beta)
            if child_score < score:
                score = child_score
                placement = child[0]
            if score <= alpha:
